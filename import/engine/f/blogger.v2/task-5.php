...
$content = fetch('https://www.blogger.com/feeds/' . $query['blog'] . '/' . $query['parent'] . '/comments/default?alt=json&max-results=' . $query['chunk'] . '&start-index=' . $index);
...
if (!empty($data['feed']['entry'])) {
    foreach ($data['feed']['entry'] as $v) {
        $file = is_file($f = $folder . DS . 'lot' . DS . 'comment' . DS . 'blog' . DS . $n . DS . date('Y-m-d-H-i-s', strtotime($v['published']['$t'])) . '.page');
    ...
    if ($create > 0) {
        $log[microtime()] = [
            'status' => 201,
            'description' => i('%d comment' . (1 === $create ? "" : 's') . ' successfully imported to %s', [$create, '<code>' . strtr($folder . DS . 'lot' . DS . 'comment' . DS . 'blog' . DS . $n, [ROOT => '.']) . '</code>']),
            'parent' => $id
        ];
    }
}
